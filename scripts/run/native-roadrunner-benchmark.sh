...
scenario="${1:-hello}"
runtime_config="${2:-${NATIVE_ROADRUNNER_RUNTIME_CONFIG:-baseline}}"
repetition="${3:-1}"
...
base_url="http://127.0.0.1:${NATIVE_ROADRUNNER_HTTP_PORT:-8089}"
target_url="http://${NATIVE_ROADRUNNER_APP_HOST:-native-roadrunner}:${NATIVE_ROADRUNNER_APP_PORT:-80}/${scenario}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
case_id="native__${php_version}__roadrunner__${runtime_config}__${scenario}__plain__r${repetition}"
raw_output_path="results/raw/${timestamp}__${case_id}.wrk.txt"
metadata_path="results/raw/${timestamp}__${case_id}.json"

env NATIVE_ROADRUNNER_RUNTIME_CONFIG="${runtime_config}" ${compose} up -d --build native-roadrunner >/dev/null
