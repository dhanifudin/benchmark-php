...
scenario="${1:-hello}"
runtime_config="${2:-${LARAVEL_SWOOLE_RUNTIME_CONFIG:-baseline}}"
repetition="${3:-1}"
...
base_url="http://127.0.0.1:${LARAVEL_SWOOLE_HTTP_PORT:-8092}"
target_url="http://${LARAVEL_SWOOLE_APP_HOST:-laravel-swoole}:${LARAVEL_SWOOLE_APP_PORT:-80}/${scenario}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
case_id="laravel__${php_version}__swoole__${runtime_config}__${scenario}__plain__r${repetition}"
raw_output_path="results/raw/${timestamp}__${case_id}.wrk.txt"
metadata_path="results/raw/${timestamp}__${case_id}.json"

env LARAVEL_SWOOLE_RUNTIME_CONFIG="${runtime_config}" ${compose} up -d --build laravel-swoole >/dev/null
