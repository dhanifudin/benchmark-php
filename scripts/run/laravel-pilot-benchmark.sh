...
scenario="${1:-hello}"
runtime_config="${2:-${LARAVEL_PHP_RUNTIME_CONFIG:-baseline}}"
repetition="${3:-1}"
...
case "$code_form" in
  plain)
    app_host="${LARAVEL_APP_HOST:-laravel-nginx}"
    app_port="${LARAVEL_APP_PORT:-80}"
    health_port="${LARAVEL_HTTP_PORT:-8081}"
    compose_service_php="laravel-php-fpm"
    compose_service_nginx="laravel-nginx"
    compose_rt_config_var="LARAVEL_PHP_RUNTIME_CONFIG"
    framework_version="latest-supported"
    ;;
  obfuscated-supported-minimal)
    scripts/run/laravel-obfuscate.sh laravel-minimal >/dev/null
    app_host="${LARAVEL_OBFUSCATED_APP_HOST:-laravel-obfuscated-nginx}"
    app_port="${LARAVEL_OBFUSCATED_APP_PORT:-80}"
    health_port="${LARAVEL_OBFUSCATED_HTTP_PORT:-8084}"
    compose_service_php="laravel-obfuscated-php-fpm"
    compose_service_nginx="laravel-obfuscated-nginx"
    compose_rt_config_var="LARAVEL_OBFUSCATED_PHP_RUNTIME_CONFIG"
    framework_version="laravel-yakpro-minimal"
    ;;
  *)
    printf 'SKIPPED Laravel code_form not yet implemented: %s\n' "$code_form"
    exit 0
    ;;
esac
...
case_id="laravel__${php_version}__php-fpm__${runtime_config}__${scenario}__${code_form}__r${repetition}"
