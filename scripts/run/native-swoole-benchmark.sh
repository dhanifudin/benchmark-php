...
scenario="${1:-hello}"
runtime_config="${2:-${NATIVE_SWOOLE_RUNTIME_CONFIG:-baseline}}"
repetition="${3:-1}"
...
base_url="http://127.0.0.1:${NATIVE_SWOOLE_HTTP_PORT:-8090}"
target_url="http://${NATIVE_SWOOLE_APP_HOST:-native-swoole}:${NATIVE_SWOOLE_APP_PORT:-80}/${scenario}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
case_id="native__${php_version}__swoole__${runtime_config}__${scenario}__plain__r${repetition}"
raw_output_path="results/raw/${timestamp}__${case_id}.wrk.txt"
metadata_path="results/raw/${timestamp}__${case_id}.json"

env NATIVE_SWOOLE_RUNTIME_CONFIG="${runtime_config}" ${compose} up -d --build native-swoole >/dev/null
