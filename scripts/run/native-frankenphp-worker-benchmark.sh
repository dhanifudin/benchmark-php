...
scenario="${1:-hello}"
runtime_config="${2:-${NATIVE_FRANKENPHP_WORKER_RUNTIME_CONFIG:-baseline}}"
repetition="${3:-1}"
...
base_url="http://127.0.0.1:${NATIVE_FRANKENPHP_WORKER_HTTP_PORT:-8088}"
target_url="http://${NATIVE_FRANKENPHP_WORKER_APP_HOST:-native-frankenphp-worker}:${NATIVE_FRANKENPHP_WORKER_APP_PORT:-80}/${scenario}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
case_id="native__${php_version}__frankenphp-worker__${runtime_config}__${scenario}__plain__r${repetition}"
raw_output_path="results/raw/${timestamp}__${case_id}.wrk.txt"
metadata_path="results/raw/${timestamp}__${case_id}.json"

env NATIVE_FRANKENPHP_WORKER_RUNTIME_CONFIG="${runtime_config}" ${compose} up -d --build native-frankenphp-worker >/dev/null
