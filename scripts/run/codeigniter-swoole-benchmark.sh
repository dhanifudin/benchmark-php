...
scenario="${1:-hello}"
runtime_config="${2:-${CODEIGNITER_SWOOLE_RUNTIME_CONFIG:-baseline}}"
repetition="${3:-1}"
...
base_url="http://127.0.0.1:${CODEIGNITER_SWOOLE_HTTP_PORT:-8093}"
target_url="http://${CODEIGNITER_SWOOLE_APP_HOST:-codeigniter-swoole}:${CODEIGNITER_SWOOLE_APP_PORT:-80}/${scenario}"
timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
case_id="codeigniter__${php_version}__swoole__${runtime_config}__${scenario}__plain__r${repetition}"
raw_output_path="results/raw/${timestamp}__${case_id}.wrk.txt"
metadata_path="results/raw/${timestamp}__${case_id}.json"

env CODEIGNITER_SWOOLE_RUNTIME_CONFIG="${runtime_config}" ${compose} up -d --build codeigniter-swoole >/dev/null

until curl -fsS "${base_url}/healthz" >/dev/null 2>&1; do
  sleep 1
done

cache_state="$(case "$scenario" in db-read-cache-warm|db-list-cache-warm) printf warm ;; *) printf none ;; esac)"
